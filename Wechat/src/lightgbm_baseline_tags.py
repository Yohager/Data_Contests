...
TAG_EMBED_DIR = "two_cols_tags.csv"
LARGE_TAG_DIR = "350_cols_tags.csv"
FEED_LATENT_DIR = "feed_latent_data.csv"
...
def global_features(df):
    ...
    df_keywords_tags = pd.read_csv(TAG_EMBED_DIR)
    ...
    #将feed的latent的向量信息加进去
#     df_feed_latent = pd.read_csv(FEED_LATENT_DIR)
#     df = df.merge(df_feed_latent,on='feedid',how='left')
    ## 内存够用的不需要做这一步
...
##################### 全量训练 #####################
def total_train(train,test,y_list,r_list,u_auc_list,w_uauc,seeds=[2021]):
...
if __name__ == "__main__":
    ...
    #添加滑动窗口的特征
    df = sliding_windows(df,n_days=[5], max_day=15)
    #添加全局特征
